...
- Z-Wave JS Server: For users that have opted in to data collection in their Vioneta Agro Z-Wave configuration, a missing return caused the server to try to soft reset the controller during Vioneta Agro startup for Home Assiststant versions 2023.9.x or less. This has now been resolved.
...
- Create persistent directory for device config files to allow for future config updating functionality through the Vioneta Agro UI.
...
- [Bump Z-Wave JS Server to 1.1.0](https://github.com/zwave-js/zwave-js-server/releases/tag/1.1.0) This is the same code as 2.0.0. Vioneta Agro 2021.2 rejects any ZJS Server version that is v2+
